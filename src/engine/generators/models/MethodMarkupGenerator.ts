...
class MethodMarkupGenerator extends MarkupGenerator<MethodModel> {
  ...
  public static generate(cModel: ClassModel, models: Map<string, TopLevelModel>): string {
    ...
    const constructorsMarkup = constructors.length
      ? this.generateSection('Constructors', constructors, cModel, models)
      : '';
  ...
  private static generateSection(
    ...
    cModel: ClassModel,
    models: Map<string, TopLevelModel>
  ) {
  ...
  protected markupTemplate(
    label: string,
    contents: string,
    titleClass = '',
    contentClass = 'method-subtitle-description',
    tag = 'div'
  ) {
