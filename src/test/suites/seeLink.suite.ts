...
export const createSeeLinkSuite = (files: ITestFile[]) => {
  suite('See Links', function () {
    ...
    test('Should correctly parse and link "@see" method-link syntax', function () {
      ...
      assert.equal(
        onclick,
        "goToLocation('TEST_ArrayUtils.html#TEST_ArrayUtils.qsort')",
        '@see tag incorrectly parsed method link.'
      );
    });
    ...
    test('Should correctly handle "@see" link with invalid identifier', function () {
      ...
      assert.equal(
        tooltip,
        'A valid link could not be created with this identifier.',
        '@see tag incorrectly handled broken link.'
      );
    });
