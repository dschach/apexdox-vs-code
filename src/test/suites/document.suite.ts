...
export const createDocumentSuite = (files: ITestFile[]) => {
  suite('Document', () => {
    ...
    test('Should place classes in correct groups according to "@group" tags', function () {
      ...
      assert.deepEqual(
        miscellaneousGroup,
        expectedMiscellaneousGroup,
        'Menu items in assertion group do match match expected items'
      );
      assert.deepEqual(
        featureGroup,
        expectedFeatureGroup,
        'Menu items in assertion group do match match expected items'
      );
      assert.deepEqual(
        assertionGroup,
        expectedAssertionsGroup,
        'Menu items in assertion group do match match expected items'
      );
    });
    ...
    test('Should correctly link each menu item to its corresponding doc page', function () {
      ...
      $('li.nav-item')
        ...
        .forEach((menuObj) => {
          assert.ok(
            menuObj.onclick.includes(menuObj.text + '.html'),
            "Menu item's onclick attribute does not link to correct file."
          );
        });
    });
    ...
    test('Should capture tag values over multiple lines', function () {
      ...
      assert.equal(
        ...
        'author value does not match expected multi-line value'
      );
      ...
      assert.equal(
        ...
        'since value does not match expected multi-line value'
      );
